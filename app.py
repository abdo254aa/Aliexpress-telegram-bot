...
# سنبقي على حقول السعر لجلبها من ال API
...
def clean_aliexpress_url(url: str, product_id: str) -> str | None:
    ...
    try:
        ...
        base_url = urlunparse(('https', netloc, path_segment, '', '', ''))
        return base_url
    except ValueError:
...
# --- fetch_product_details_v2 (remains mostly the same, ensures price/currency are fetched) ---
async def fetch_product_details_v2(product_id: str) -> dict | None:
    ...
    def _execute_api_call():
        try:
            ...
            # QUERY_FIELDS includes price and currency now
    ...
    try:
        ...
        # ... (response structure checks same as before) ...
        ...
        if resp_code != 200:
             ...
             # Try scraping as fallback ONLY if API response code indicates product not found or similar issue
             # Specific error codes might need identification here. For now, assume any non-200 might benefit from scrape.
             logger.info(f"Attempting scrape fallback due to non-200 API response for {product_id}")
             ...
             try:
                 ...
                 if scraped_name:
                      logger.info(f"Successfully scraped details after non-200 API response for product ID: {product_id}")
                      product_info = {'title': scraped_name, 'image_url': scraped_image, 'price': None, 'currency': None, 'source': 'Scraped'}
                      await product_cache.set(product_id, product_info)
                      return product_info
                 else:
                     logger.warning(f"Scraping also failed after non-200 API response for product ID: {product_id}")
                     return None
             except Exception as scrape_err:
                logger.error(f"Error during scraping fallback after non-200 API response for {product_id}: {scrape_err}")
                return None
        ...
        if not products:
            logger.warning(f"No products found in API response for ID {product_id} (Resp Code 200)")
            # Try scraping if API returns 200 but no product data
            logger.info(f"Attempting scrape fallback after empty product list in API response for {product_id}")
            # --- Fallback logic repeated for consistency ---
            try:
                 loop_inner = asyncio.get_event_loop()
                 scraped_name, scraped_image = await loop_inner.run_in_executor(
                    executor, get_product_details_by_id, product_id
                 ...
                 if scraped_name:
                      logger.info(f"Successfully scraped details after empty API product list for product ID: {product_id}")
                      product_info = {'title': scraped_name, 'image_url': scraped_image, 'price': None, 'currency': None, 'source': 'Scraped'}
                      await product_cache.set(product_id, product_info)
                      return product_info
                 else:
                     logger.warning(f"Scraping also failed after empty API product list for product ID: {product_id}")
                     return None
            except Exception as scrape_err:
                logger.error(f"Error during scraping fallback after empty API product list for {product_id}: {scrape_err}")
                return None
            # --- End of fallback logic ---

        product_data = products[0]
        # Extract price and currency along with other details
        product_info = {
            'image_url': product_data.get('product_main_image_url'),
            'price': product_data.get('target_sale_price'), # Get the price
            'currency': product_data.get('target_sale_price_currency', TARGET_CURRENCY), # Get the currency
        ...
        # Consider scraping as a fallback here too if parsing fails unexpectedly
        return None # Return None on other parsing errors
...
async def generate_affiliate_links_batch(target_urls: list[str]) -> dict[str, str | None]:
    ...
    for url in target_urls:
        ...
        if cached_link:
            ...
            results_dict[url] = None; uncached_urls.append(url)
    ...
    if not response or not response.body: logger.error(f"Batch link generation API call failed or returned empty body for {len(uncached_urls)} URLs."); return results_dict
    try:
        response_data = response.body
        if isinstance(response_data, str): response_data = json.loads(response_data)
        if 'error_response' in response_data: error_details = response_data.get('error_response', {}); logger.error(f"API Error for Batch Link Generation: Code={error_details.get('code', 'N/A')}, Msg={error_details.get('msg', 'Unknown API error')}"); return results_dict
        generate_response = response_data.get('aliexpress_affiliate_link_generate_response')
        if not generate_response: logger.error(f"Missing 'aliexpress_affiliate_link_generate_response' key. Response: {response_data}"); return results_dict
        resp_result_outer = generate_response.get('resp_result')
        if not resp_result_outer: logger.error(f"Missing 'resp_result' key. Response: {generate_response}"); return results_dict
        resp_code = resp_result_outer.get('resp_code')
        if resp_code != 200: logger.error(f"API response code not 200 for batch link generation. Code: {resp_code}, Msg: {resp_result_outer.get('resp_msg', 'Unknown')}"); logger.error(f"Failed URLs (request): {uncached_urls}"); return results_dict
        result = resp_result_outer.get('result', {}); links_data = result.get('promotion_links', {}).get('promotion_link', [])
        if not result or not links_data or not isinstance(links_data, list): logger.warning(f"No 'result' or valid 'promotion_links' in batch response. Response: {resp_result_outer}"); return results_dict
        expiry_date = datetime.now() + timedelta(days=CACHE_EXPIRY_DAYS); api_returned_links_map = {}
        for link_info in links_data:
            if isinstance(link_info, dict):
                ...
                if source_url_returned and promo_link:
                     if source_url_returned in uncached_urls: api_returned_links_map[source_url_returned] = promo_link
                     else: logger.warning(f"Received link for an unexpected source_value from API: {source_url_returned}")
                else: logger.warning(f"Missing 'source_value' or 'promotion_link' in batch response item: {link_info}")
            else: logger.warning(f"Promotion link data item is not a dictionary: {link_info}")
        for url in uncached_urls:
            if url in api_returned_links_map: promo_link = api_returned_links_map[url]; results_dict[url] = promo_link; await link_cache.set(url, promo_link); logger.debug(f"Cached affiliate link for {url} until {expiry_date.strftime('%Y-%m-%d %H:%M:%S')}")
            else: logger.warning(f"No affiliate link returned or processed from batch API for requested URL: {url}")
        return results_dict
    except Exception as e: logger.exception(f"Error parsing batch link generation response: {e}"); return results_dict
...
async def _get_product_data(product_id: str) -> tuple[dict | None, str]:
    ...
    details_source = "None"
    if product_details:
        details_source = product_details.get('source', 'API') # Source can be API or Scraped now
        ...
        # This case means both API and scraping failed in fetch_product_details_v2
...
async def _generate_offer_links(base_url: str) -> dict[str, str | None]:
    ...
    for offer_key in OFFER_ORDER:
        if offer_key in OFFER_PARAMS:
            ...
            else: logger.warning(f"Could not build target URL for offer {offer_key} with base {base_url}")
        else: logger.warning(f"Offer key '{offer_key}' from OFFER_ORDER not found in OFFER_PARAMS.")
    if not urls_to_fetch: logger.warning(f"No target URLs could be built for base URL: {base_url}"); return {}
    ...
    for offer_key, target_url in target_urls_map.items():
        ...
        if not promo_link: logger.warning(f"Failed to get affiliate link for offer {offer_key} (target: {target_url})")
    return generated_links


# --- MODIFIED _build_response_message ---
def _build_response_message(product_data: dict, generated_links: dict) -> str:
    """Builds the Arabic response message string including price per link."""
    message_lines = []
    ...
    if product_price and product_currency:
        # Format as (USD 123.45) - Adjust if currency needs symbol instead of code
        price_string = f"({product_currency} {product_price})"
    elif product_data.get('source') == 'Scraped':
        price_string = "(السعر غير متاح عبر API)" # Indicate price unavailability from scraping
    # else: price is None from API or source is 'None' - price_string remains ""
    ...
    # --- Build Message ---
    # 1. Product Name
    message_lines.append(f"📝 <b>إسم المنتج :</b> {product_title[:250]}")
    message_lines.append("\n✳️ <b>قارن الأسعار واكتشف أرخص سعر للمنتج ⬇️🤩</b>\n")

    # 2. Offer Links with Price
    offers_found = False
    for offer_key in OFFER_ORDER:
        ...
        if link and label:
            # Add price string if available
            price_part = f" بـ : {price_string}" if price_string else ""
            # Construct the line with label, optional price, emoji, and link
            message_lines.append(f"{label}{price_part} 🔥:\n{link}\n")
            offers_found = True

    # Handle case where NO offers were found
    if not offers_found:
        # Provide context if product data was fetched but links failed
        if product_data.get('source') != 'None':
...
async def _send_telegram_response(context: ContextTypes.DEFAULT_TYPE, chat_id: int, product_data: dict, message_text: str):
    ...
    product_id = product_data.get('id', 'N/A') # Get product ID for logging
    ...
    try:
        if product_image:
            ...
            # Send as text if no image (e.g., scraping failed for image or API didn't provide one)
            logger.info(f"Sending text response for product {product_id} as image was not available.")
            await context.bot.send_message(
                chat_id=chat_id,
                text=message_text,
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True, # Good practice for messages with multiple links
            )
...
async def process_product_telegram(product_id: str, base_url: str, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # 1. Get Product Data (API/Scrape) - Now includes price/currency
        product_data, details_source = await _get_product_data(product_id)

        # Check if product_data exists and source is not 'None' (meaning API or scraping succeeded at least partially)
        ...
        product_data['id'] = product_id # Add ID for logging/error messages if not already present

        # 2. Generate Affiliate Links
        generated_links = await _generate_offer_links(base_url)

        # 3. Build Response Message (Arabic, includes price per link)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ... (message handling logic remains largely the same) ...
    ...
    processed_product_ids = set(); tasks = []
    async with aiohttp.ClientSession() as session:
        for url in potential_urls:
            original_url = url; product_id = None; base_url = None
            if not url.startswith(('http://', 'https://')):
                 if COMBINED_DOMAIN_REGEX.search(url): url = f"https://{url}"; logger.debug(f"Prepending https://: {url}")
                 else: logger.debug(f"Skipping URL: {original_url}"); continue
            if STANDARD_ALIEXPRESS_DOMAIN_REGEX.match(url):
                ...
                if final_url:
                    ...
                    else: logger.warning(f"Could not extract ID from resolved URL: {final_url}")
            ...
            elif product_id and product_id in processed_product_ids: logger.debug(f"Skipping duplicate product ID: {product_id}")
            # Optional: Log other failure cases if needed
...
# --- Main Function ---
def main() -> None:
    """Starts the bot (Arabic only, with price per link)."""
    ...
    # Message handler for non-link text
    ...
    # --- Log startup info ---
    logger.info("Starting Telegram bot polling (Arabic Interface, Price per Link)...")
