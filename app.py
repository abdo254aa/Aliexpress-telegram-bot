...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Sends the Arabic welcome message, all bold."""
    # Wrap the entire message content within <b> tags
    welcome_message = """<b>
...
def _build_response_message(product_data: dict, generated_links: dict) -> str:
    """Builds the Arabic response message string, all bold."""
    message_lines = []
    # Escape HTML characters in title to prevent conflicts with <b> tags
    import html
    ...
    # 2. Offer Links (Label Bold, Link not explicitly bolded - clients handle link styling)
    offers_found = False
    for offer_key in OFFER_ORDER:
        ...
        if link and label:
            # Wrap label in <b>, link remains as is
            message_lines.append(f"<b>{label}</b>\n{link}\n")
            offers_found = True

    # Handle case where NO offers were found (Bold)
    if not offers_found:
    ...
    try:
        if product_image:
            await context.bot.send_photo(
                chat_id=chat_id,
                photo=product_image,
                caption=message_text, # Caption already contains <b> tags
                parse_mode=ParseMode.HTML,
            )
        else:
            await context.bot.send_message(
                chat_id=chat_id,
                text=message_text, # Text already contains <b> tags
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True,
            )
    except Exception as send_error:
        logger.error(f"Failed to send message for product {product_id} to chat {chat_id}: {send_error}")
        # Fallback message in Arabic and Bold
        try:
            fallback_text = f"<b>⚠️ حدث خطأ أثناء عرض المنتج {product_id}. يرجى المحاولة مرة أخرى.</b>"
            await context.bot.send_message(
                chat_id=chat_id,
                text=fallback_text,
                parse_mode=ParseMode.HTML
...
async def process_product_telegram(product_id: str, base_url: str, update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Processes a single product ID and sends the response in Arabic, all bold."""
    ...
    try:
        ...
        if not product_data or details_source == "None":
             ...
             # Send user-facing error in Arabic and Bold
             await context.bot.send_message(
                 chat_id=chat_id,
                 text=f"<b>❌ تعذر استرداد بيانات المنتج ذي المعرف {product_id}.</b>",
                 parse_mode=ParseMode.HTML
             )
             return
        ...
        try:
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"<b>حدث خطأ غير متوقع أثناء معالجة المنتج {product_id}. عذراً!</b>",
                parse_mode=ParseMode.HTML
            )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not potential_urls:
        # Send non-link prompt in Arabic and Bold
        await context.bot.send_message(
            chat_id=chat_id,
            text="<b>يرجى إرسال رابط منتج AliExpress لإنشاء تخفيضات له.</b>",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    if not tasks:
        ...
        # Send error message in Arabic and Bold
        await context.bot.send_message(
            chat_id=chat_id,
            text="<b>❌ لم نتمكن من العثور على أي روابط منتجات AliExpress صالحة في رسالتك.</b>",
            parse_mode=ParseMode.HTML
        )
    else:
        if len(tasks) > 1:
            # Send processing message in Arabic and Bold only if multiple items
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"<b>⏳ جاري معالجة {len(tasks)} منتجات AliExpress. يرجى الانتظار...</b>",
                parse_mode=ParseMode.HTML
            )
...
def main() -> None:
    """Starts the bot (Arabic only, All Bold)."""
    ...
    # Message handler for text that doesn't contain AE links (Arabic prompt, Bold)
    ...
    logger.info("Starting Telegram bot polling (Arabic Interface, All Bold)...")
